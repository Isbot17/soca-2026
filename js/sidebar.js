...
(() => {
  ...
  document.querySelectorAll('.nav a').forEach(link => {
    link.addEventListener('click', async (e) => {
      ...
      if (targetUrl === currentPage) return e.preventDefault(); 
      e.preventDefault(); 
      
      const innerContent = document.querySelector('.content');
      const currentTopbar = document.querySelector('.topbar');
      
      if (!innerContent) return window.location.href = targetUrl; // Fallback

      try {
        // Setup Loader
        let topLoader = document.getElementById('top-loader') ?? (() => {
        ...
        topLoader.className = '';
        void topLoader.offsetWidth; // Trigger reflow
        topLoader.className = 'start';
        innerContent.classList.add('fade-out');
        
        // Fetch new page in parallel with transition delay
        ...
        const fetchedTopbar = doc.querySelector('.topbar');
        
        if (!fetchedContent) throw new Error("Content not found");

        // Swap Content
        innerContent.innerHTML = fetchedContent.innerHTML;
        
        // Sync Topbar
        if (currentTopbar && fetchedTopbar) {
            const currentTitle = currentTopbar.querySelector('h2, h1');
            const fetchedTitle = fetchedTopbar.querySelector('h2, h1');
            if (currentTitle && fetchedTitle) currentTitle.innerHTML = fetchedTitle.innerHTML;

            const currentBreadcrumb = currentTopbar.querySelector('.breadcrumb');
            const fetchedBreadcrumb = fetchedTopbar.querySelector('.breadcrumb');
            if (currentBreadcrumb && fetchedBreadcrumb) currentBreadcrumb.innerHTML = fetchedBreadcrumb.innerHTML;
        }

        // Update URL state
        const displayUrl = targetUrl.replace('.html', '');
        window.history.pushState({ path: targetUrl }, '', displayUrl);
        currentPage = targetUrl;
        updateActiveLink(targetUrl);

        // Scroll to top
        const scrollContainer = document.querySelector('.main') || document.querySelector('.main-content');
        if (scrollContainer) scrollContainer.scrollTop = 0;

        // Modern script re-execution using the spread operator
        [...innerContent.querySelectorAll('script')].forEach(oldScript => {
        ...
        // Finish Loader
        topLoader.className = 'done';
        innerContent.classList.remove('fade-out');
        
        setTimeout(() => {
          topLoader.className = 'hidden';
          setTimeout(() => topLoader.className = '', 400); 
        }, 300);

      } catch (error) {
        console.error("SPA routing failed, falling back to hard reload.", error);
